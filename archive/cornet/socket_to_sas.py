...
# Globals blocked: Used to ensure recieved socket messages display on the terminal before the main menu blocks the
# command-line interface . This variable is set to 'True' right before a client emit with an expected response
# executes. The response will set it back to False.
__blocked = False
...
parser = ArgumentParser(
    description='CORNET-to-SAS Client Script - Provide a server address and port in order to connect to the SAS.')
...
def _compileMeasReport(node, reportingAlgorithm="every64Bins"):
    ...
    reportingAlgorithm : string
        ...
        "oneChannelAtATime" - Averages the bins that are exclusive to each received 10MHz channel into 1
        RcvdPowerMeasReport
...
def simCreateNode(requests):
    ...
    TODO: Make sure the data can create a valid USRP
    TODO: What params would RX need instead?
    TODO: Decide what KeyErrors should halt node creation
...
def simRegistrationReq(requests):
    ...
    for request in requests:
        ...
        if not (cbsdSerialNumber := node.getSerialNumber()):
            print("No cbsdSerialNumber found for the node with IP Address: '" + str(
                node.getIpAddress()) + "'. Registration Request invalid.")
            continue
...
def configRegistrationReq():
    ...
    Motivation for this is to allow the hands on command line interface without the tedious form filling.
    """
    return [None]
...
def cmdRegistrationReq():
    ...
    Note: UHD Lib provides the serial, addr, and model ('type' for the uhd lib) for all usrps. Nodes with an FPGA
    includes the fpga.
    ...
    for _ in range(num):
        ...
        installationInfoSelector = getSelectorBoolean(
            input("Do you want to enter Device Installation Information (Y)es or (N)o: "))
        if installationInfoSelector:
            installationParam = promptInstallationParam()
        measCapability = promptMeasCapability()
        groupingParam = None
        groupingParamSelector = getSelectorBoolean(
            input("Would you like to enter Grouping Parameter Info? (Y)es or (N)o: "))
        if groupingParamSelector:
            quantity = int(input(
                "How many groups do you want to create for this node?: "))  # TODO: Ensure non-negative (can this be 0?)
...
def registrationRequest(clientio, payload=None):
    ...
    if __sim_mode:
        ...
        while True:
            user_input = input(
                "Would you like to manually enter the registraion info or load from a file? (E)nter or (L)oad or ("
                "exit): ")
...
def simSpectrumInquiryReq(requests):
    ...
    for request in requests:
        ...
        if not (cbsdId := node.getCbsdId()):
            print(
                "No cbsdId found for the node with IP Address: '" + node.getIpAddress() + "'. Spectrum Inquiry "
                                                                                          "Request invalid.")
            continue
        inquiredSpectrum = []
...
def cmdSpectrumInquiryReq():
    ...
    provideRcvdPowerMeas = getSelectorBoolean(
        input("Do you want to provide Received Power Measurments to the SAS? (Y)es or (N)o: "))
    measReport = None
...
def spectrumInquiryRequest(clientio, payload=None):
    ...
    if __sim_mode:
        ...
        while True:
            dataSource = input(
                "Would you like to manually enter the Spectrum Inquiry Request info or load from a file? (E)nter or ("
                "L)oad: ")
...
def grantRequest(clientio, payload=None):
    ...
    if __sim_mode:
        ...
        while (dataSource := input(
                "Would you like to manually enter the Grant Request info or load from a file? (E)nter or (L)oad: ")):
            if dataSource == 'E' or dataSource == 'e':
    ...
    for grantResponse in grantResponses:
        ...
        if grantExpireTime := _grabPossibleEntry(grantResponse, "grantExpireTime"):
            ...
            try:
                ...
                print("SASError: SAS sent grantExpireTime as '" + str(
                    grantExpireTime) + "' which is not in " + timeFormat + " format. Grant invalid.")
                continue
        else:
...
def scheduleNextHeartbeat(clientio, node):
    ...
    """
    delayTilNextHeartbeat = float(
        node.getGrant().getHeartbeatInterval()) * 0.9  # Send heartbeats a little sooner than the interval
...
def simHeartbeatReq(requests):
    ...
    for request in requests:
        ...
        if not (cbsdId := node.getCbsdId()):
            print(
                "No cbsdId found for the node with IP Address: '" + node.getIpAddress() + "'. Heartbeat Request "
                                                                                          "invalid.")
            continue
...
def heartbeatRequest(clientio, node=None, payload=None):
    ...
    if payload:
        ...
        while (dataSource := input(
                "Would you like to manually enter the Heartbeat Request info or load from a file? (E)nter or (L)oad: "
                "")):
            if dataSource == 'E' or dataSource == 'e':
    ...
    for hbResponse in hbResponses:
        ...
        if cbsdId:
            if node := findNodeAwaitingResponseByCbsdId(cbsdId):  # Find the Node with the given CBSD ID
                node.stopHbTimer()  # TODO: For MultipleGrants - Should this be stopped when the grantId is matched
                # or just the Node?
                nodes_awaiting_response.remove(node)
                if grantId:
                    if (not (
                            node.getGrant().getGrantId() == grantId)):  # Find if the Node has a Grant with the given
                        # Grant ID
                        print(
                            "This Node with CBSD ID: '" + cbsdId + "' does not have a Grant with Grant ID: '" +
                            grantId + "'.")
                        isIncompleteResponse = True
                else:
                    if responseCode == "0":
                        print(
                            "SASError: Missing required parameter: grantId. Cannot match this response to a Grant "
                            "without a Grant ID.")
                    else:
                        print(
                            "Missing conditional parameter: grantId. Cannot match this response to a Grant without a "
                            "Grant ID.")
                    isIncompleteResponse = True
            else:
        ...
        # TODO: grantExpireTime is required when (responseCode is 0 or 501) and (the heartbeat request asked to renew
        #  the Grant)
...
def simRelinquishmentReq(requests):
    ...
    for request in requests:
        ...
        if not node:
            print(
                "No Registered Node was found with the IP Address: '" + node.getIpAddress() + "'. Relinquishment "
                                                                                              "Request invalid.")
            continue
        cbsdId = node.getCbsdId()
        if not cbsdId:
            print(
                "No cbsdId found for the node with IP Address: '" + node.getIpAddress() + "'. Relinquishment Request "
                                                                                          "invalid.")
            continue
        grantId = node.getGrant().getGrantId()
        nodes_awaiting_response.append(node)
...
def relinquishmentRequest(clientio, payload=None):
    ...
    if __sim_mode:
        ...
        while (dataSource := input(
                "Would you like to manually enter the Relinquishment Request info or load from a file? (E)nter or ("
                "L)oad: ")):
            if dataSource == 'E' or dataSource == 'e':
...
def handleRelinquishmentResponse(clientio, data):
    ...
    if not (relinquishResponses := _grabPossibleEntry(jsonData, "relinquishmentResponse")):
        print(
            "Unreadable data. Expecting JSON formatted payload with key 'relinquishmentResponse'. Relinquishment(s) "
            "invalid.")
        return
        ...
        if cbsdId:
            if node := findNodeAwaitingResponseByCbsdId(cbsdId):
                if grantId:
                    ...
                    if responseCode == "0":
                        print(
                            "Missing required parameter: grantId. Cannot match this response to a Grant without a "
                            "Grant ID.")
                    else:
                        print(
                            "Missing conditional parameter: grantId. Cannot match this response to a Grant without a "
...
def simDeregistrationReq(requests):
    ...
    for request in requests:
        ...
        if not node:
            print(
                "No Created Node was found with the IP Address: '" + node.getIpAddress() + "'. Deregistration Request "
                                                                                           "invalid.")
            continue
        cbsdId = node.getCbsdId()
        if not cbsdId:
            print(
                "No cbsdId found for the node with IP Address: '" + node.getIpAddress() + "'. Deregistration Request "
                                                                                          "invalid.")
            continue
        nodes_awaiting_response.append(node)
...
def deregistrationRequest(clientio, payload=None):
    ...
    if __sim_mode:
        ...
        while (dataSource := input(
                "Would you like to manually enter the Deregistration Request info or load from a file? (E)nter or ("
                "L)oad: ")):
            if dataSource == 'E' or dataSource == 'e':
...
def handleDeregistrationResponse(clientio, data):
    ...
    if not (deregistrationResponses := _grabPossibleEntry(jsonData, "deregistrationResponse")):
        print(
            "Unreadable data. Expecting JSON formatted payload with key 'deregistrationResponse'. Deregistration(s) "
            "invalid.")
        return
    iter = 0
        ...
        if node := findRegisteredNodeByCbsdId(cbsdId):
            if (lowFreq := safeCast(_grabPossibleEntry(data,
                ...
                safeCast(_grabPossibleEntry(data,
                    ...
                    print(
                        "SAS Attempted 'updateRxParams' but no valid integer 'highFrequency' found. Ignoring SAS call.")
            else:
                print("SAS Attempted 'updateRxParams' but no valid integer 'lowFrequency' found. Ignoring SAS call.")
        else:
            print(
                "SAS Attempted 'updateRxParams' but no registered Node found with cbsdId '" + cbsdId + "'. Ignoring "
                                                                                                       "SAS call.")
    else:
